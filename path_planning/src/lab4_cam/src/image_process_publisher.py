...
import camera_subscriber #<---- THIS IS VERY IMPORTANT
...
if __name__ == '__main__':
  ...
  #rospy.wait_for_service('images_and_pointcloud')
  
  # Initializes the image processing node
  rospy.init_node('image_processing_testing_node')
  
  # Creates a function used to call the 
  # image capture service: ImageSrv is the service type
  #last_image_service = rospy.ServiceProxy('images_and_pointcloud', ImageSrv)
  realsenseRGB = last_image_service()[0]
  np_image = ros_to_np_img(realsenseRGB)
  ...
  while not rospy.is_shutdown() and False:
    ...
    try:
      ...
      realsenseRGB = last_image_service().logitech_rgb
      print("HELLO!!!")
      # Convert the ROS message to a NumPy image
      np_image = ros_to_np_img(realsenseRGB)
      #print("THIS SI THIGN",np_image)
      print("this is running")
